...
def generate_report_data(store):
    ...
    tz = store.timezone_str or 'America/Chicago' # 'America/Chicago'
    target_timezone = pytz_timezone(tz) # <class 'pytz.tzfile.America/Chicago'> (America/Chicago)
    
    # hard coding current time as max of all the logs
    time = StoreStatusLog.objects.all().order_by('-timestamp').first().timestamp  # 2023-01-25 18:13:22.479220+00:00
    local_time = time.astimezone(target_timezone) # 2023-01-25 12:13:22.479220-06:00
    utc_timezone = pytz_timezone('UTC') # UTC
    utc_time = time.astimezone(utc_timezone) # 2023-01-25 18:13:22.479220+00:00
